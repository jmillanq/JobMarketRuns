/* 
	...
	Last Modify		:	131020
		...
		interacting the violence variable with any continuos variable.

		eg. I can interact violence with the number of household members.
*/
...
// Import macros
	...
	loc prefix $PREFIX
	loc intVar $INTER
	loc intLabel : var label `intVar'

// Titles 
	loc tableTitle "`yVar' from `lAgeOn' to `uAgeOn' on `vioVar' - Violence and `intLabel'" 
...
// Gen interaction contVar*zHom
	cap drop zHom_*
	g zHom_`intVar' = zHom*`intVar'
...
// First Stage - Vio = Drug Traffic
	...
	eststo SS: ///
    	logit `yVar' zHom zHom_`intVar' ControlF* `dur1' $control ///
...
foreach out in csv tex {
	esttab SS using ///
	LatexFiles/`prefix'Drt`dep'`lA'`uA'on`vioVar'DT`set'poli`cfpoli'intW`intVar'.`out' ///
	, r `esttabOpt'  keep(zHom zHom_`intVar' `intVar' ControlF*) ///
	s(F_Instr CTest r2_p M Ind N, ///
	fmt(2 2 2 0 0 0)) ///
	addnotes( ///
		"Duration var : `labelY'" ///
		"Conditional on : `condVar'" ///
		"Interaction with :	`intLabel'" ///
